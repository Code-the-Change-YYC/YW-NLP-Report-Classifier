...
const findStaffInitials = (desc) =>
	new Set(
		(desc.match(/\b[Ss]taff (?!AM|PM)([A-Z]{2})\b/g) || []).map((match) => match.split(" ")[1])
	);
...
const useAutocomplete = ({
	...
	const checkServices = useCallback(
		(desc) => {
			if (services) {
				setServicesInvolvedAutocomplete(autocompleteMultipleOptions(desc, services));
			}
		},
	...
	const checkInvolvesChild = useCallback(
		(desc) => {
			if (childInvolvedOptions) {
				const involvesChild = autocompleteSingleOption(desc, childInvolvedOptions);
				if (involvesChild) {
	...
	const checkInvolvesNonClient = useCallback(
		(desc) => {
			if (guestInvolvedOptions) {
				const involvesNonClient = autocompleteSingleOption(desc, guestInvolvedOptions);
	...
	const checkSecondInitials = useCallback(
		(desc, clientInitials) => {
			...
			const found = desc.match(/\b(?!AM|PM)([A-Z]{2})\b/g);
			const staff = new Set(
				(desc.match(/\b[Ss]taff (?!AM|PM)([A-Z]{2})\b/g) || []).map(
					(match) => match.split(" ")[1]
				)
			);
